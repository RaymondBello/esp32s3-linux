...
# esptool.py --chip esp32s3 -p /dev/tty.wchusbserial54ED0024551 -b 921600 --before=default_reset --after=hard_reset write_flash 0x0 bootloader.bin 0x10000 network_adapter.bin 0x8000 partition-table.bin
...
# /app/build/build-xtensa-2023.02-fdpic-esp32s3/images get etc/ partition from here

# esptool.py -p /dev/tty.wchusbserial54ED0024551  -b 921600 --before default_reset --after hard_reset --chip esp32s3  write_flash --flash_mode dio --flash_size detect --flash_freq 80m 0x0 bootloader.bin 0x8000 partition-table.bin 0x10000 network_adapter.bin

# parttool.py write_partition --partition-name etc --input build-buildroot-esp32s3/images/etc.jffs2

# Editing boot arguments from "root=mtd:rootfs" -> "root=mtdblock0"
# dcp esp32s3.dtsi :/app/build/buildroot/dl/linux/git/arch/xtensa/boot/dts/esp32s3.dtsi